...
function getMobileOperatingSystem(){var a=navigator.userAgent||navigator.vendor||window.opera;return/windows phone/i.test(a)?"Windows Phone":/android/i.test(a)?"Android":/iPad|iPhone|iPod/.test(a)&&!window.MSStream?"iOS":"unknown"}STATUS.MOBILEOS=getMobileOperatingSystem();STATUS.ISMOBILE=!1;-1==STATUS.MOBILEOS.indexOf("unknown")&&(STATUS.ISMOBILE=!0);var SOCKETSERVER="https://exalandsocket.spime.im",noSleep=new NoSleep;HLMain={};var frameCount=0,millis=0,delta=0;
...
window.innerHeight);STATUS.FPC&&HL.controls.handleResize()});AA=STATUS.ISVISUAL?new AAMS:{}}var performanceLow=0,performanceHigh=0;
function mainLoop(){HLMain.rafID=STATUS.VR?HL.VREffect.requestAnimationFrame(mainLoop):window.requestAnimationFrame(mainLoop);frameCount++;millis=frameCount/60;delta=HL.clock.getDelta();HLE.MIRROR||HLE.WATER||HLAnim.sea();HLE.MIRROR&&HLAnim.mirrorWaves();HLE.WATER&&HLAnim.seaGLSL();HLAnim.landGLSL();HLAnim.models();HLAnim.particles();STATUS.ISMOBILE||STATUS.VR||STATUS.REMIDI?HL.controls.update():STATUS.FPC&&HL.controls.update(delta);HLE.WATER&&HL.materials.water.render();HLE.MIRROR&&HL.materials.mirror.render();
STATUS.FBO||1==HLR.glitchEffect?(STATUS.CARDBOARD?(HL.stereoEffect.render(HL.scene,HL.camera,HL.glitchFBO),HL.renderer.crop(0,0,.5*window.innerWidth,window.innerHeight),HL.renderer.render(HL.glitchScene,HL.glitchCamera),HL.renderer.crop(.5*window.innerWidth,0,.5*window.innerWidth,window.innerHeight)):HL.renderer.render(HL.scene,HL.camera,HL.glitchFBO),HL.renderer.render(HL.glitchScene,HL.glitchCamera)):STATUS.CARDBOARD?HL.stereoEffect.render(HL.scene,HL.camera):HL.renderer.render(HL.scene,HL.camera);
delta=null}var setVisibility=function(a,d){for(var c=document.querySelectorAll(a),b=0;b<c.length;b++)c[b].style.opacity=d?1:0,c[b].style.display=d?"block":"none"};
