  ...
  function mainLoop() {
    ...
    HLRemote.updateHLParams(0.5,0.25,0.25,0.05);// .2+Math.random()*.4, .2+Math.random()*.4, .2+Math.random()*.4 );//), AA.getFreq(64), AA.getFreq(200));
    ...
    if(isCardboard || isVR){
      ...
      if(isMapped){
        ...
        window.shadowMapViewer.render( HL.renderer );
    ...
    HL.materials.land.uniforms.customShadowMap.value = HL.lights.sun.shadow.map.texture;
