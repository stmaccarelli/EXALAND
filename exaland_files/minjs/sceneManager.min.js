...
HLS.scenes.standard=function(){HLS.raf=STATUS.VR?HL.VREffect.requestAnimationFrame(HLS.scenes.standard):window.requestAnimationFrame(HLS.scenes.standard);.95<HLR.fft[0]&&(HL.materials.land.uniforms.landSeed.value+=.001*HLR.fft[0]);HLE.reactiveMoveSpeed=HLE.BASE_MOVE_SPEED+7*(HLR.smoothFft[0]+HLR.smoothFft[1]+HLR.smoothFft[2])*HLE.BASE_MOVE_SPEED;HLE.moveSpeed=HLE.reactiveMoveSpeed*(STATUS.CARDBOARD||STATUS.VR?.75:1);HLE.moveSpeed*=HLE.acceleration;HLR.tempNoiseFreq=7-3*HLR.fft[2];HLE.noiseFrequency=
HLR.tempNoiseFreq;HLE.noiseFrequency2+=.3*Math.min(0,HLR.fft[1]-HLR.fft[2]);HLS.lumi=5*HLR.fft[2];HLC.horizon.setRGB(HLC.tempHorizon.r+HLS.lumi,HLC.tempHorizon.g+HLS.lumi,HLC.tempHorizon.b+HLS.lumi);if(HLS.scenesAddons[HLS.sceneId])HLS.scenesAddons[HLS.sceneId]()};HLS.initScenes.exaland=function(){console.log(" exaland scene init")};var randomDebounce1=!0,randomDebounce2=!0;function isRegisteredKick(){return!1}var elephantDebounce=!0;
...
.15*HLR.fft[2])};function pickRandomPropertyHL(){var a,b;for(b in window)if("HL"==b.substr(0,2)&&.1>Math.random()&&"HL"!=b){a=b;break}return window[a]}var cartello;
